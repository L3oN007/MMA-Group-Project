...
export default function Welcome() {
  ...
  return (
    <SafeAreaView className="flex h-full items-center justify-between bg-white">
      ...
      >
        <Text className="text-md font-pbold text-black">Skip</Text>
      </TouchableOpacity>
      <Swiper
        ref={swiperRef}
        loop={false}
        ...
        activeDot={
          <View className="mx-1 h-[4px] w-[32px] rounded-full bg-[#fc0200]" />
        }
      ...
      >
        {onboarding.map((item) => (
          <View key={item.id} className="flex items-center justify-center p-5">
            ...
            <Text className="text-md mx-10 mt-3 text-center font-psemibold text-[#858585]">
              {item.description}
            </Text>
          </View>
        ))}
